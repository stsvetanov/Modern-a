...
# Правихме го чрез метода .value_counts(), но:
import matplotlib.pyplot as plt
hist_data=df.iloc[:,0]
x_label='Избиращ номер'
y_label='Честота (в брой обаждания)'
df_title='Хистограма на изходящите обаждания по номера'
...
hist_data.plot.hist(grid=True, bins=20, rwidth=0.9,
                   color='#607c8e')
plt.title(df_title)
plt.xlabel(x_label)
plt.ylabel(y_label)
...
hdv=hist_data.value_counts()
n, labels = list(hdv),list(hdv.index.astype(str))
fig = plt.figure()
...
# ax.set_yticks(np.arange(0, maxfreq, 10))
# plt.show()
ax = hdv.plot.hist(bins=4, alpha=0.5)
plt.show()

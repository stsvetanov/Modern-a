...
for people_n in people:

    if people_n.get("gender") == "female":
        name_female = people_n.get("name")
        interests_female = people_n.get("interests")
        age_female = people_n.get("age")
        ex_female = people_n.get("ex")
    else:
        name_female = {}
        interests_female = {}
        age_female = {}
    ...
    for people_m in people:
        if people_m.get("gender") == "male":
            name_male = people_m.get("name")
            interests_male = people_m.get("interests")
            age_male = people_m.get("age")
            ex_male = people_m.get("ex")

            common_interests = set(interests_female).intersection(interests_male)
            age_dif = 3  # Разлика в годините
            if common_interests and abs(age_male - age_female) <= age_dif:
                # Дали е в списъка на бивши.
                if name_male in list(ex_female):
                    continue
                else:
                    if name_female in list(ex_male):
                        continue
                    else:
                        print(f"{name_female} + {name_male} -> {common_interests}")
